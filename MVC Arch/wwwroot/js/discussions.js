...
function filterDiscussions() {
    ...
    if (visibleItems.length === 0) {
        if (!noResultsMsg) {
            ...
            if (discussionsList) {
                discussionsList.appendChild(noResultsMsg);
            }
        }
...
function setupDiscussionActions() {
    ...
    markAnsweredButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const questionTitle = discussionItem.querySelector('.question-content h3')?.textContent || 'Question';
    ...
    replyButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            const discussionItem = this.closest('.discussion-item');
            const questionTitle = discussionItem.querySelector('.question-content h3')?.textContent || 'Question';
            const studentName = discussionItem.querySelector('.student-info h4')?.textContent || 'Student';
